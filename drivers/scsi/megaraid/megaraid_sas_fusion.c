 ...
 *
 * Driver has informed block layer by setting boundary rules for
 * BIO merging done at NVME page size boundary calling kernel API
 * blk_queue_virt_boundary inside slave_config.
 *
 * IO merges do take the virt_boundary into account so there should be no 
 * holes, even if IO merging takes place at Block layer.
 *
 * Block layer ensures if virt_boundary is set, driver should not get 
 * IO buffers with holes. This code of holes detection is kept to catch
 * any corner case of IO buffers with holes even if virt_boundary is set.
 * Driver would call kernel panic with SGE details dumped, if any hole is 
 * detected in data buffer.  
 * 
 * This code of holes detection can be removed after one internal test cycle
 * completion without this kernel panic.
 */
		...
		if ((i != 0) && (i != (sge_count - 1))) {
			if (mega_mod64(sg_dma_len(sg_scmd), mr_nvme_pg_size) ||
			    mega_mod64(sg_dma_address(sg_scmd),
				       mr_nvme_pg_size)) {
				panic("Panic inside func: %s line: %d- IO with hole is received from OS for NVMe device, SGE:%d,  SGE address:0x%llx  SGE len:%d\n",
					__func__, __LINE__, i, sg_dma_address(sg_scmd), sg_dma_len(sg_scmd));
				break;
			}
		}
