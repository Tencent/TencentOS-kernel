...
/*
 * Later versions of the kernel removed support of the
 * initialization function macros.  In this case, define
 * the macros as nops to allow the code to compile
 */
...
lpfc_cleanup(struct lpfc_vport *vport)
{
	...
	list_for_each_entry_safe(ndlp, next_ndlp, &vport->fc_nodes, nlp_listp) {
		if (!NLP_CHK_NODE_ACT(ndlp)) {
			...
			if (!ndlp)
				continue;
			spin_lock_irqsave(&phba->ndlp_lock, iflags);
			NLP_SET_FREE_REQ(ndlp);
			spin_unlock_irqrestore(&phba->ndlp_lock, iflags);
...
#if defined(BUILD_NVME)
	if (phba->nvme_io)
		queue_work(phba->nvme_io, &phba->nvme_defer_work);
#endif
}
...
lpfc_cgn_save_evt_cnt(struct lpfc_hba *phba)
{
	...
	cp->cgn_info_acr = phba->cgn_p.cgn_param_acr;
	cp->cgn_info_rxburst = phba->cgn_p.cgn_param_rxburst;
	cp->cgn_info_rxbw = phba->cgn_p.cgn_param_rxbw;
 ...
 * inteval to the current point in time. It is called from IO completion
...
lpfc_cmf_timer(struct hrtimer *timer)
{
	...
	if (phba->cmf_active_mode == LPFC_CFG_MANAGED &&
		 ...
		 * add an extra 2% slop factor, equal to LPFC_CMF_INTERVAL
		 * add an extra 1%. The goal is to equalize total with a
		 * time > LPFC_CMF_INTERVAL or <= LPFC_CMF_INTERVAL + 1
		 */
		if (ms == LPFC_CMF_INTERVAL)
			extra = div_u64(total, 100);
	...
	/* Since total_bytes has aleady been zero'ed, its okay to unblock
 ...
 *
 * This routine validates the 8 congestion parameters passed
 * by the FW to the driver via an ACQE event.  The parameters
 * are checked for in-range values.  Any value out of range
...
lpfc_cgn_params_val(struct lpfc_hba *phba, struct lpfc_cgn_param *p_cfg_param)
{
	...
	if (!lpfc_rangecheck(p_cfg_param->cgn_param_acr, LPFC_CFG_CONSERVE,
			     LPFC_CFG_SEVERE))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6226 CMF ACR param out of range: %d\n",
				 p_cfg_param->cgn_param_acr);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_rxburst,
			     LPFC_CFG_BURST_MIN, LPFC_CFG_BURST_MAX))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6227 CMF rxburst param out of range: %d\n",
				 p_cfg_param->cgn_param_rxburst);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_rxbw,
			     LPFC_CFG_BW_MIN, LPFC_CFG_BW_MAX))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6228 CMF rxbw param out of range: %d\n",
				 p_cfg_param->cgn_param_rxbw);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_acr_max_rxburst,
			     LPFC_CFG_BURST_MIN, LPFC_CFG_BURST_MAX))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6229 CMF maxACRburst param out of range: %d\n",
				 p_cfg_param->cgn_param_acr_max_rxburst);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_acr_max_rxbw,
			     LPFC_CFG_BW_MIN, LPFC_CFG_BW_MAX))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6230 CMF maxACRbw param out of range: %d\n",
				 p_cfg_param->cgn_param_acr_max_rxbw);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_acr_scale,
			     LPFC_CFG_LOW_SCALE, LPFC_CFG_HIGH_SCALE))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6231 CMF ACRscale param out of range: %d\n",
				 p_cfg_param->cgn_param_acr_scale);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_acr_min_rxburst, 1,
			     p_cfg_param->cgn_param_acr_max_rxburst))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6232 CMF minACRburst param out of range: %d\n",
				 p_cfg_param->cgn_param_acr_min_rxburst);

	if (!lpfc_rangecheck(p_cfg_param->cgn_param_acr_min_rxbw,
			     LPFC_CFG_BW_MIN,
			     p_cfg_param->cgn_param_acr_max_rxbw))
		lpfc_printf_log(phba, KERN_ERR, LOG_CGN_MGMT,
				"6233 CMF minACRbw param out of range: %d\n",
				 p_cfg_param->cgn_param_acr_min_rxbw);
	spin_unlock_irq(&phba->hbalock);
}

/**
 * lpfc_sli4_congparm_chg_evt - Process a FW cong parm Change event
 * @phba: pointer to lpfc hba data structure.
 * @p_cgn_params: pointer to a data buffer with the FW cong params.
		      ...
		      struct lpfc_cgn_param *p_cgn_param, uint32_t len)
{
	...
	if (p_cgn_param->cgn_param_magic == LPFC_CFG_PARAM_MAGIC_NUM) {
		lpfc_printf_log(phba, KERN_INFO, LOG_CGN_MGMT | LOG_INIT,
				...
				p_cgn_param->cgn_param_acr,
				p_cgn_param->cgn_param_rxburst,
				p_cgn_param->cgn_param_rxbw,
...
#if defined(BUILD_NVME)
	if (phba->cfg_enable_fc4_type & LPFC_ENABLE_NVME)
		phba->nvme_io = alloc_workqueue("lpfc_nvme_io",
						WQ_MEM_RECLAIM, 0);
#endif
...
#if defined(BUILD_NVME)
	if (phba->nvme_io) {
		flush_workqueue(phba->nvme_io);
		destroy_workqueue(phba->nvme_io);
		phba->nvme_io = NULL;
	}
#endif
...
lpfc_sli4_read_config(struct lpfc_hba *phba)
{
	...
	if (rc != MBX_SUCCESS) {
		...
		phba->sli4_hba.fawwpn_in_use =
			bf_get(lpfc_mbx_rd_conf_fawwpn_inuse, rd_config);
		...
		if (lpfc_use_cgn_signal) {
			...
			if (bf_get(lpfc_mbx_rd_conf_acs, rd_config)) {
				...
				if (phba->cgn_reg_signal !=
				    LPFC_CGN_SIGNAL_WARN) {
					/* Must support both or none */
					phba->cgn_reg_fpin = LPFC_CGN_FPIN_BOTH;
					phba->cgn_reg_signal =
						LPFC_CGN_SIGNAL_NO_SPT;
				} else {
					phba->cgn_reg_signal =
						LPFC_CGN_SIGNAL_BOTH;
					phba->cgn_reg_fpin =
						LPFC_CGN_FPIN_NONE;
				}
			}
...
lpfc_sli4_queue_setup(struct lpfc_hba *phba)
{
	...
	if (phba->sli4_hba.cq_max) {
		...
		phba->sli4_hba.cq_lookup = kzalloc((phba->sli4_hba.cq_max + 1) *
...
static int lpfc_cpu_offline(unsigned int cpu, struct hlist_node *node)
{
	...
	if (!phba) {
		WARN_ONCE(!phba, "cpu: %u. phba:%px.", raw_smp_processor_id(),
			  phba);
		return 0;
	}
...
/**
 * _lpfc_reg_congestion_buf - register congestion info buffer with HBA
 * @phba: Pointer to hba context object.
 * reg: flag to determine register or unregister.
 */
int
_lpfc_reg_congestion_buf(struct lpfc_hba *phba, int reg)
{
...
{
	lpfc_cmf_stop(phba);
	return _lpfc_reg_congestion_buf(phba, 0);
}

int
lpfc_reg_congestion_buf(struct lpfc_hba *phba)
{
	return _lpfc_reg_congestion_buf(phba, 1);
}
 ...
 *
 * This function is called in the SLI4 code path to read the port's
 * sli4 capabilities. Called on proble from lpfc_sli4_driver_resource_setup.
 *
 * This function may be be called from any context that can block-wait
 * for the completion.  The expectation is that this routine is called
 * typically from probe_one or from the online routine.
...
fcponly:
	...
	INIT_LIST_HEAD(&phba->nvme_pendq);
	INIT_LIST_HEAD(&phba->nvme_abts_pendq);
	spin_lock_init(&phba->nvme_pendq_lock);
	INIT_WORK(&phba->nvme_defer_work, lpfc_nvme_issue_pend_io);
#endif
	phba->nvme_pendq_cnt = 0;
	return 0;
}
