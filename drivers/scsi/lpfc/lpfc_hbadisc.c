...
lpfc_register_remote_port(struct lpfc_vport *vport, struct lpfc_nodelist *ndlp)
{
	...
	if (ndlp->nlp_flag & NLP_EXTERNAL_DIF)
		   ...
		   int state)
{
	...
	lpfc_printf_vlog(vport, KERN_INFO, LOG_NODE,
			 "0904 NPort state transition %p x%06x x%x: %s -> %s\n",
	...
	uint32_t did)
{
	...
	setup_timer(&ndlp->dhc_cfg.nlp_reauthfunc, lpfc_auth_reauth,
		    (unsigned long)ndlp);
#else
	timer_setup(&ndlp->dhc_cfg.nlp_reauthfunc, lpfc_auth_reauth,
		    0);
#endif
#endif
		 ...
		 int state)
{
	...
	did = ndlp->nlp_DID;
	flag = (ndlp->nlp_flag & (NLP_EXTERNAL_DIF | NLP_UNREG_INP));
...
lpfc_mbx_cmpl_fdmi_reg_login(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb)
{
	 ...
	 */
	if (vport->port_type == LPFC_PHYSICAL_PORT) {
		vport->fc_flag &= ~FC_CT_CGN_RPA; /* For extra ELXE2EM RPA */
