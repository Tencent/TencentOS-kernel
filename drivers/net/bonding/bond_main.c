...
static bool bond_should_notify_peers(struct bonding *bond)
{
	...
	if ((bond->params.broadcast_arp || bond->params.broadcast_nd) &&
		...
		/*
		 * For BOND_MODE_8023AD, curr_active_slave could be NULL,
		 * we use first_slave instead.
		 * BOND_MODE_ACTIVEBACKUP also use this function,
		 * in that case, you could close broadcast_arp and broadcast_nd.
		 */
...
static int bond_3ad_xor_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* Broadcast arp or nd to all slaves. */
