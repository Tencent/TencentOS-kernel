					   ...
					   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d", &new_value) != 1) {
		pr_err("%s: no broadcast_arp value specified.\n",
		       bond->dev->name);
		ret = -EINVAL;
		goto out;
	}
	...
	if (new_value != 0 && new_value != 1) {
		pr_err("%s: Invalid broadcast_arp value %d not in range 0-1; rejected.\n",
		       bond->dev->name, new_value);
		ret = -EINVAL;
		goto out;
	}
	pr_info("%s: Setting broadcast_arp to %d.\n",
		bond->dev->name, new_value);
...
static DEVICE_ATTR(broadcast_arp, S_IRUGO | S_IWUSR,
		   bonding_show_broadcast_arp,
		   bonding_store_broadcast_arp);
