...
#define DRV_MODULE_VERSION	"1.10.1" DRV_MODULE_EXTRA_VER
...
#define BNXT_VPD_FLD_LEN	32
	...
	#define BNXT_FLAG_USR_RSS_HASH_KEY	0x80000000
...
#define BNXT_CHIP_SUPPORTS_PHY(bp)	(BNXT_ASIC(bp) || BNXT_CHIP_SR2(bp))
	...
	u32			ulp_version;	/* New bnxt_re checks that
						 ...
						 * ulp_probe.
						 */
#define BNXT_ULP_VERSION 0x695a0004	/* Change this when any interface
...
#define BNXT_STATE_DRV_REGISTERED	7
	...
	#define BNXT_FW_CAP_LINK_ADMIN			0x00000020
	#define BNXT_FW_CAP_VF_RES_MIN_GUARANTEED	0x00000040
	#define BNXT_FW_CAP_KONG_MB_CHNL		0x00000080
	#define BNXT_FW_CAP_ADMIN_MTU			0x00000100
	#define BNXT_FW_CAP_ADMIN_PF			0x00000200
	#define BNXT_FW_CAP_OVS_64BIT_HANDLE		0x00000400
	#define BNXT_FW_CAP_TRUSTED_VF			0x00000800
	#define BNXT_FW_CAP_VF_VNIC_NOTIFY		0x00001000
	...
	#define BNXT_FW_CAP_SECURE_MODE			0x00080000
					 ...
					 struct bnxt_db_info *db, u32 idx)
{
	if (bp->flags & BNXT_FLAG_CHIP_P5) {
		writeq_relaxed(db->db_key64 | DB_RING_IDX(db, idx),
			       db->doorbell);
	} else {
...
{
	if (bp->flags & BNXT_FLAG_CHIP_P5) {
		writeq(db->db_key64 | DB_RING_IDX(db, idx), db->doorbell);
	} else {
		u32 db_val = db->db_key32 | DB_RING_IDX(db, idx);
