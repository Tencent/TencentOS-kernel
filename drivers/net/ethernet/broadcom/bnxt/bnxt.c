			     ...
			     u32 vlan_tag_flags, u32 cfa_action)
{
	...
	*end = 0;
	...
	writeq(db->db_key64 | DBR_TYPE_PUSH_START | DB_RING_IDX(db, prod),
	       db->doorbell);
...
static netdev_tx_t bnxt_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (free_size == bp->tx_ring_size && length <= bp->tx_push_thresh) {
		...
		if (bp->flags & BNXT_FLAG_CHIP_P5)
			rc = bnxt_push_xmit_p5(bp, txr, txq, skb,
					       vlan_tag_flags, cfa_action);
		else
			rc = bnxt_push_xmit(bp, txr, txq, skb, vlan_tag_flags,
					    cfa_action);
		if (!rc)
			goto tx_done;
...
normal_tx:
	...
	if (length < BNXT_MIN_PKT_SIZE) {
		...
		if (skb_pad(skb, pad)) {
			/* SKB already freed. */
			tx_buf->skb = NULL;
			txr->bnapi->cp_ring.sw_stats.tx_sw_errors++;
			return NETDEV_TX_OK;
		}
	...
	*event |= BNXT_RX_EVENT;
	...
	if (rxcmp1->rx_cmp_cfa_code_errors_v2 & RX_CMP_L2_ERRORS) {
		...
		if (rx_err & RX_CMPL_ERRORS_BUFFER_ERROR_MASK) {
			bnapi->cp_ring.sw_stats.rx_buf_errors++;
			if (!(bp->flags & BNXT_FLAG_CHIP_P5)) {
	...
	if (RX_CMP_L4_CS_OK(rxcmp1)) {
		...
		if (rxcmp1->rx_cmp_cfa_code_errors_v2 & RX_CMP_L4_CS_ERR_BITS) {
			if (dev->features & NETIF_F_RXCSUM)
				bnapi->cp_ring.sw_stats.rx_l4_csum_errors++;
		}
	}
...
static int bnxt_alloc_tx_rings(struct bnxt *bp)
{
	...
	bp->tx_push_size = 0;
	if (bp->tx_push_thresh && !(bp->flags & BNXT_FLAG_CHIP_P5)) {
		int push_size;
...
static int bnxt_alloc_stats(struct bnxt *bp)
{
	...
	bp->hw_rx_port_stats =
		dma_alloc_coherent(&pdev->dev, bp->hw_port_stats_size,
				   &bp->hw_rx_port_stats_map, GFP_KERNEL);
	if (!bp->hw_rx_port_stats)
		return -ENOMEM;
...
int bnxt_hwrm_l2_filter_alloc(struct bnxt *bp, struct bnxt_l2_filter *fltr)
{
	...
	bnxt_hwrm_cmd_hdr_init(bp, &req, HWRM_CFA_L2_FILTER_ALLOC, -1,
			       target_id);
			     ...
			     u32 map_idx, u32 xid)
{
	...
	db = &txr->tx_push_db;
	db->doorbell = NULL;
	if (!(bp->flags & BNXT_FLAG_CHIP_P5) || !bp->tx_push_thresh ||
	    !bp->db_base_wc)
		return;
	...
	db->db_key64 |= (u64)xid << DBR_XID_SFT;
	txr->tx_push_wcb = bp->db_base_wc + ((dpi - 1) * DB_WCB_PAGE_SIZE) +
...
static int bnxt_hwrm_ver_get(struct bnxt *bp)
{
	...
	if (bp->hwrm_spec_code > 0x10803 && resp->hwrm_fw_major >= 1)
		snprintf(bp->fw_ver_str, FW_VER_STR_LEN, "%d.%d.%d.%d",
			 resp->hwrm_fw_major, resp->hwrm_fw_minor,
			 resp->hwrm_fw_build, resp->hwrm_fw_patch);
	else
	...
	if (!BNXT_ASIC(bp) && bp->pdev->devfn != 0) {
...
static int bnxt_map_db_bar(struct bnxt *bp)
{
	...
	bp->bar1 = ioremap_nocache(pci_resource_start(bp->pdev, 2),
				   bp->db_size_nc);
	if (!bp->bar1)
		return -ENOMEM;
	return 0;
