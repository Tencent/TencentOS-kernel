...
#define BNXT_DB_NQ_P5(db, idx)						\
	writeq((db)->db_key64 | DBR_TYPE_NQ | DB_RING_IDX(db, idx),	\
	       (db)->doorbell)
...
#define BNXT_DB_NQ_ARM_P5(db, idx)					\
	writeq((db)->db_key64 | DBR_TYPE_NQ_ARM | DB_RING_IDX(db, idx),	\
	       (db)->doorbell)
...
{
	if (bp->flags & BNXT_FLAG_CHIP_P5)
		writeq(db->db_key64 | DBR_TYPE_CQ_ARMALL | DB_RING_IDX(db, idx),
		       db->doorbell);
	else
		BNXT_DB_CQ(db, idx);
}
...
static netdev_tx_t bnxt_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(i >= bp->tx_nr_rings)) {
		...
		netdev_warn(dev, "TX packet queue %d exceeds maximum %d\n",
			    i, bp->tx_nr_rings - 1);
		return NETDEV_TX_OK;
	}
...
normal_tx:
	...
	if (length < BNXT_MIN_PKT_SIZE) {
		...
		if (skb_pad(skb, pad)) {
			/* SKB already freed. */
			tx_buf->skb = NULL;
			txr->bnapi->cp_ring.sw_stats.tx.tx_sw_errors++;
			return NETDEV_TX_OK;
		}
...
#if defined(HAVE_NETDEV_XMIT_MORE) || defined(HAVE_SKB_XMIT_MORE)
	...
	if (cmp_type == CMP_TYPE_RX_L2_CMP) {
		...
		if (RX_CMP_VLAN_VALID(rxcmp)) {
			...
			if (tpid_sel == RX_CMP_METADATA1_TPID_8021Q) {
				...
				vtag = RX_CMP_METADATA0_VID(rxcmp1);
				return vtag | OLD_VLAN_VALID;
			}
		}
	}
...
#else
static void bnxt_rx_vlan(struct sk_buff *skb, u8 cmp_type,
			 struct rx_cmp *rxcmp, struct rx_cmp_ext *rxcmp1)
{
	u16 vtag, vlan_proto;
	u32 meta_data;
		       ...
		       u32 *raw_cons, u8 *event)
{
	...
	*event |= BNXT_RX_EVENT;
	...
	if (unlikely((flags & RX_CMP_FLAGS_ITYPES_MASK) ==
		...
		if (bp->flags & BNXT_FLAG_CHIP_P5)
			bnxt_get_rx_ts_p5(bp, &ts, rxcmp1->rx_cmp_unused3);
		else
...
#else
		bnxt_rx_vlan(skb, cmp_type, rxcmp, rxcmp1);
#endif
	skb_checksum_none_assert(skb);
	...
	while (1) {
		...
		if (cmp_type == CMP_TYPE_TX_L2_CMP) {
			...
			if (unlikely(tx_pkts > bp->tx_wake_thresh)) {
			   ...
			   cmp_type <= CMP_TYPE_RX_TPA_AGG_CMP) {
			if (likely(budget))
...
static int bnxt_busy_poll(struct napi_struct *napi)
{
	...
	if (bp->flags & BNXT_FLAG_CHIP_P5) {
		...
		if (cpr2)
			rx_work = bnxt_poll_work(bp, cpr2, budget);
		cpr2 = cpr->cp_ring_arr[1];
		if (cpr2 && rx_work < budget)
...
static void bnxt_init_vnics(struct bnxt *bp)
{
	...
	for (i = 0; i < bp->nr_vnics; i++) {
		...
		if (bp->vnic_info[i].rss_hash_key) {
			if (i == 0) {
				if (bp->flags & BNXT_FLAG_USR_RSS_HASH_KEY) {
...
static int bnxt_alloc_stats(struct bnxt *bp)
{
	...
	if (!BNXT_ASIC(bp) && !(bp->flags & BNXT_FLAG_CHIP_SR2))
		return 0;
...
static int bnxt_alloc_ntp_fltrs(struct bnxt *bp)
{
	...
	bp->ntp_fltr_count = 0;
	bp->ntp_fltr_bmap = kcalloc(BITS_TO_LONGS(bp->max_fltr), sizeof(long),
				    GFP_KERNEL);
			     ...
			     u32 map_idx, u32 xid)
{
	...
	case BNXT_PUSH_MODE_PPP:
		...
		db->doorbell = bp->db_base_wc + map_idx * 2 * DB_PPP_SIZE;
...
static int bnxt_hwrm_ring_alloc(struct bnxt *bp)
{
	...
	for (i = 0; i < bp->tx_nr_rings; i++) {
		...
		bnxt_set_push_db(bp, txr, map_idx, ring->fw_ring_id);
	}
...
static int __bnxt_hwrm_ptp_qcfg(struct bnxt *bp)
{
	...
	if (!(bp->flags & BNXT_FLAG_CHIP_P5)) {
		ptp->rx_regs[BNXT_PTP_RX_TS_L] =
			le32_to_cpu(resp->rx_ts_reg_off_lower);
		ptp->rx_regs[BNXT_PTP_RX_TS_H] =
			le32_to_cpu(resp->rx_ts_reg_off_upper);
		ptp->rx_regs[BNXT_PTP_RX_SEQ] =
			le32_to_cpu(resp->rx_ts_reg_off_seq_id);
		ptp->rx_regs[BNXT_PTP_RX_FIFO] =
			le32_to_cpu(resp->rx_ts_reg_off_fifo);
		ptp->rx_regs[BNXT_PTP_RX_FIFO_ADV] =
			le32_to_cpu(resp->rx_ts_reg_off_fifo_adv);
		ptp->tx_regs[BNXT_PTP_TX_TS_L] =
			le32_to_cpu(resp->tx_ts_reg_off_lower);
		ptp->tx_regs[BNXT_PTP_TX_TS_H] =
			le32_to_cpu(resp->tx_ts_reg_off_upper);
		ptp->tx_regs[BNXT_PTP_TX_SEQ] =
			le32_to_cpu(resp->tx_ts_reg_off_seq_id);
		ptp->tx_regs[BNXT_PTP_TX_FIFO] =
			le32_to_cpu(resp->tx_ts_reg_off_fifo);
	}
...
static int bnxt_hwrm_func_backing_store_cfg(struct bnxt *bp, u32 enables)
{
	...
	rc = hwrm_req_init(bp, req, HWRM_FUNC_BACKING_STORE_CFG);
	if (rc)
		return rc;
...
static int bnxt_alloc_ctx_mem(struct bnxt *bp)
{
	 ...
	 * allocation by f/w.
	 */
	num_mr = min_t(u32, ctx->mrav_max_entries / 2, 1024 * 256);
...
skip_rdma:
	...
	for (i = 0; i < ctx->tqm_fp_rings_count + 1; i++) {
		...
		rc = bnxt_alloc_ctx_pg_tbls(bp, ctx_pg, mem_size, 2, false);
		if (rc)
			return rc;
...
static int bnxt_hwrm_ver_get(struct bnxt *bp)
{
	...
	if (!BNXT_ASIC(bp) && !BNXT_CHIP_SR2(bp) && bp->pdev->devfn != 0) {
...
static int bnxt_hwrm_port_qstats_ext(struct bnxt *bp, u8 flags)
{
	...
	req_qs->rx_stat_host_addr =
		cpu_to_le64(bp->rx_port_stats_ext.hw_stats_map);
	tx_stat_size = bp->tx_port_stats_ext.hw_stats ?
		       sizeof(struct rx_port_stats_ext) : 0;
	req_qs->tx_stat_size = cpu_to_le16(tx_stat_size);
	req_qs->tx_stat_host_addr =
		cpu_to_le64(bp->tx_port_stats_ext.hw_stats_map);
...
static int bnxt_update_link(struct bnxt *bp, bool chng_link_state)
{
	...
	link_info->lp_auto_pam4_link_speeds =
		le16_to_cpu(resp->link_partner_pam4_adv_speeds);
			      ...
			      struct device_attribute *devattr, char *buf)
{
	...
	if (!hwrm_req_init(bp, req, HWRM_TEMP_MONITOR_QUERY)) {
		resp = hwrm_req_hold(bp, req);
...
static inline void bnxt_hwmon_close(struct bnxt *bp)
{
}

static inline void bnxt_hwmon_open(struct bnxt *bp)
{
}
...
retry_if_change:
	...
	if (rc == -EAGAIN && timeout < 40) {
		msleep(50);
		timeout++;
		goto retry_if_change;
	} else if (rc) {
		return rc;
	}
					   ...
					   netdev_features_t features)
{
	...
	if ((features & BNXT_HW_FEATURE_VLAN_ALL_RX) !=
		...
		else if (features & BNXT_HW_FEATURE_VLAN_ALL_RX)
			features |= BNXT_HW_FEATURE_VLAN_ALL_RX;
	}
#ifdef CONFIG_BNXT_SRIOV
	if (BNXT_VF(bp) && bp->vf.vlan)
			 ...
			 u32 *reg_buf)
{
	...
	dbg_reg_buf = hwrm_req_dma_slice(bp, req, HWRM_DBG_REG_BUF_SIZE,
					 &dma_handle);
	if (!dbg_reg_buf) {
		rc = -ENOMEM;
	...
	for (i = 0; i < num_words; ) {
		...
		/* holding response is sufficient for repeated commands too */
		rc = hwrm_req_send(bp, req);
...
skip_rxr:
	...
	netdev_info(bp->dev, "[%d]: TBD{prod: %x cons: %x cp prod: %x cp cons: %x} "
		    "RBD{prod: %x cons: %x cp prod: %x cp cons: %x} "
...
#ifdef HAVE_FLOW_HASH_FROM_KEYS
...
static const struct net_device_ops bnxt_netdev_ops = {
	...
	.ndo_bpf		= bnxt_xdp,
#endif
#if defined(HAVE_XDP_FRAME) && !defined(HAVE_EXT_NDO_XDP_XMIT)
...
read_sn:
	pos = pci_vpd_find_info_keyword(vpd_data, i, ro_size, "SN");
	if (pos < 0)
		goto exit;
