...
static int __read_mostly sysrq_use_leftctrl = 1;
...
static void sysrq_reinject_alt_sysrq(struct work_struct *work)
{
	...
	if (sysrq->need_reinject) {
		...
		sysrq_code = sysrq_use_leftctrl ? KEY_LEFTCTRL : KEY_SYSRQ;
				  ...
				  unsigned int code, int value)
{
	...
	case KEY_LEFTCTRL:
		if (sysrq_use_leftctrl && !sysrq->active) {
			...
			if (sysrq->active)
				clear_bit(KEY_LEFTCTRL, sysrq->handle.dev->key);
